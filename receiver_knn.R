...
receivers<-read_csv("receivers.csv") %>%
  mutate(cfbref_id=player_id)
receivers$playerid <- NULL

receivers[is.na(receivers)] = 0
receivers <- receivers %>%
...
rm(combine_data)
receivers <- merge(avs, receivers, by="cfbref_id") %>%
  filter(av>0) %>%
  filter(rec>0)

x_data <- receivers[6:21]
x_stdv <- apply(x_data, 2, sd)
x_scaled <- scale(x_data,center=T, x_stdv)
y_data <- as.vector(receivers[3])
...
#broad jump is not seemingly a valuable measure, so we shall ignore it
weighted_x_scaled$broad_jump <- NULL
project_wrs<-function(index){
  weight_exponent <- 1
  print(receivers[index,][1])
  player_info <- receivers[index,][1:3]
  ...
  for (player_index in 1:k_value){
    id <- receivers[model$nn_index[player_index],][1:3]
    weight <- 1/model$distances[player_index]^weight_exponent
...
wr_projections<-drop_na(reduce(lapply(1:nrow(receivers), project_wrs), 
                               rbind))
hist(qb_projections$predicted_av)
cor(wr_projections$predicted_av, wr_projections$av)
write_csv(wr_projections, "receiver_example.csv")
