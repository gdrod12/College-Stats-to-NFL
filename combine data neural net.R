#repeating process for only combine data
library(tidyverse)
library(nflreadr)
library(keras)
library(tensorflow)
...
predict_draft_class <- function(test_year){
  ...
  test_scaled <- as.matrix(data.frame(scale(testqbs[c(6:14)], center=x_mins, x_maxs-x_mins)))
...
otherdrafts <- lapply(c(1989, 1994, 2001, 2007, 2011, 2014), predict_draft_class)
otherdrafts <- reduce(otherdrafts, rbind)
draft_classes<-rbind(draft_classes %>%
                       filter(!season %in% c(1989, 1994, 2001, 2007, 2011, 2014)), otherdrafts)
draft1990 <- predict_draft_class(1990)
draft_classes <- rbind(draft_classes %>%
                       filter(season != 1990),
                     draft1990)
rm(drafT_classes)
...
write_csv(data.frame(as.matrix(draft_data)), "combinedraft_rankings.csv")
